...
| 6   | **Release & rollback**              | ✅ Compliant   | `.github/workflows/release.yml` runs Release Please: Conventional Commits open a rotating release PR that bumps `package.json` + `CHANGELOG.md`; merging tags the commit, publishes the GitHub Release and attaches the audited `dist/` archive. **Rollback = redeploy an earlier tag** (re-run Deploy on that ref) or revert-and-push. |
...
None — the remaining nice-to-haves (signed tags, changelog-driven release notes polish) are
handled well enough by Release Please for a study project.
